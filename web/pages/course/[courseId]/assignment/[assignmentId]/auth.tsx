...
import { useProjectAuth } from "@/hooks/api/assignment/student-project/use-project-auth";
import { fetchClient } from "@/utils/api";
import { protectRoute } from "@/utils/auth";
import { GetServerSidePropsContext } from "next";
import { useRouter } from "next/router";

export default function AssignmentAuthenticationPage() {
  const router = useRouter();
  const { assignmentId } = router.query;
  const { authData } = useProjectAuth(assignmentId as unknown as number);
    ...
    <div className="flex flex-col mx-auto w-full max-w-[1200px] px-4 gap-8 my-8">
      ...
      {!!authData && (
        <div className="flex flex-col gap-3">
          <p className="text-sm font-bold">Auth public key</p>
          <p className="text-sm text-accent-foreground/80">
            Use the key below to verify and decode authentication tokens from
            Dabblebase in your projects.
          </p>
          <CopyText
            className="w-[500px]"
            text={authData.auth_public_key}
...
AssignmentAuthenticationPage.getLayout = function getLayout(
