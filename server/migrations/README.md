...
the database is created and dropped with the `setup_dev` script.
...
2. Bring the database up to date with `main` and the latest migration:
   1. `python3 -m server.scripts.setup_dev`
   2. `alembic stamp head`
3. Run the migration generation script:
   `python3 -m server.scripts.generate_migration <remote> <branch>`
4. Review the generated migration by looking at the new file added to `server/migration/versions`
...
1. `git switch main`
2. `python3 -m server.scripts.setup_dev`
